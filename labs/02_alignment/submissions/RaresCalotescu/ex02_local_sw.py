...
# ===================== Inițializare matrice locală =====================

def init_score_matrix_local(m: int, n: int):
    """Matrice (m+1) x (n+1) plină cu 0 pentru SW."""
...
def score_cell_local(score, i: int, j: int, a: str, b: str, match: int, mismatch: int, gap: int):
    """
    Scor SW pentru (i, j):
      diag  = score[i-1][j-1] + (match dacă a==b altfel mismatch)
      up    = score[i-1][j] + gap
      left  = score[i][j-1] + gap
    Rezultat = max(0, diag, up, left)
    """
...
def smith_waterman(seq1: str, seq2: str, match=3, mismatch=-3, gap=-2):
    ...
    # Umplere + reținere celulă cu scor maxim
    ...
    # Backtracking din celula cu scor maxim până când scorul devine 0
    align1, align2 = "", ""
    i, j = max_pos
...
def main():
    ...
    ap.add_argument("--match", type=int, default=3)
    ap.add_argument("--mismatch", type=int, default=-3)
    ...
    a1, a2, sc = smith_waterman(s1, s2, match=args.match, mismatch=args.mismatch, gap=args.gap)
